...
def run_log_reg(word_group_name, cited_paper_table_name, pub_year_cutoff):
    ...
    if os.path.exists("regression_X_"+word_group_name+".pkl"):
        ...
        if not os.path.exists("reg_cited_id_index_dict_"+word_group_name+".pkl"):
            cursor.execute("SELECT distinct cited_paper_level_"+word_group_name+".cited_id FROM cited_paper_level_"+word_group_name+" INNER JOIN cited_papers ON cited_paper_level_"+word_group_name+".cited_id=cited_papers.cited_id WHERE cited_papers.year IS NOT NULL AND cited_papers.ncit07 IS NOT NULL and cited_papers.ncit IS NOT NULL")
